...
const chill = (event, context, callback) => {
  if (!process.env.GithubAccessToken)
    throw new Error('Github token must be set via GithubAccessToken env vars');
  if (!process.env.SlackHookURL)
    throw new Error('Slack hook URL must be set via SlackHookURL env vars');
...
const getDaysInRow = data => {
  return new Promise(resolve => {
    ...
    if (daysInRow >= process.env.Streak) {
      resolve(
        `Your GitHub contribution streak is ${daysInRow} days 🙀\n\n*Time to take a break!*`
      );
    } else resolve();
  });
};
...
const getEvents = data => {
  return new Promise(resolve => {
    let events = process.env.EventsJSON
      ? JSON.parse(process.env.EventsJSON)
      : data;
    ...
    if (exclude.length > 0)
      events = _.filter(events, item => exclude.indexOf(item.type) == -1);
    resolve(events);
  });
};
